...
def setup(bot):
    ...
    async def distribute_cmd(interaction: discord.Interaction, participants_file: discord.Attachment, items_csv: discord.Attachment):
        ...
        try:
            ...
            # ✅ Final announcement embed with GMT+8 timestamp in MM-DD-YYYY format
            gmt8 = timezone(timedelta(hours=8))
            now = datetime.now(gmt8).strftime("%A, %m-%d-%Y %H:%M:%S GMT+8")

            announcement = discord.Embed(
                title="📢 Clan Sanctuary Loot",
                description="@everyone All loot from Clan Sanctuary is ready.\n\nPlease wait for the Leader and Elders to handle the final distribution.",
                color=0x00ff00
            )
            announcement.set_footer(text=f"Prepared on {now}")
